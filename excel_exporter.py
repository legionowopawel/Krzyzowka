    ...
    def export(self, grid: CrosswordGrid, filepath: str) -> bool:
        ...
        try:
            ...
            for num, clue, word in h_clues:
                position_str = ""
                for placed_word, w_row, w_col, direction, _ in grid.placed_words:
                    if (
                        placed_word.upper() == word.upper()
                        and direction == Direction.HORIZONTAL
                    ):
                        position_str = f"H{w_row + 1}_{w_col + 1}. "
                        break
                ws[f"A{row}"] = f"{position_str}{num}. {clue} ({len(word)} liter)"
            ...
            for num, clue, word in v_clues:
                ...
                for placed_word, w_row, w_col, direction, _ in grid.placed_words:
                    if (
                        placed_word.upper() == word.upper()
                        and direction == Direction.VERTICAL
                    ):
                        position_str = f"V{w_row + 1}_{w_col + 1}. "
                        break
                ws[f"A{row}"] = f"{position_str}{num}. {clue} ({len(word)} liter)"
                ws[f"A{row}"].font = Font(name="Arial", size=10)
                row += 1

            # Dodaj wyrazy użyte w krzyżówce
            row += 2
            ws[f"A{row}"] = "WYRAZY UŻYTE W KRZYŻÓWCE (alfabetycznie)"
            ws[f"A{row}"].font = Font(name="Arial", size=12, bold=True)

            row += 1
            all_words = set()
            for placed_word, _, _, _, _ in grid.placed_words:
                all_words.add(placed_word.upper())
